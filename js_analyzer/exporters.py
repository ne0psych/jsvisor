#!/usr/bin/env python3
"""
Enhancement #6 — Output & Reporting: SARIF, Postman, Markdown exporters

- SARIF for GitHub Code Scanning
- Postman collection of all endpoints
- Markdown summary with risk levels
...
def generate_sarif(findings: dict, target: str) -> dict:
    ...
    for cat, items in findings.items():
        ...
        rule_id = f"js-analyzer/{cat}"
        if rule_id not in rule_index:
            rule_index[rule_id] = len(rules)
            rules.append({
                "id": rule_id,
                "name": cat.replace('_', ' ').title(),
                "shortDescription": {"text": f"JS Analyzer: {cat}"},
        ...
        "runs": [{
            "tool": {
                "driver": {
                    "name": "JS Analyzer",
                    "version": "4.0.0",
                    "informationUri": "https://github.com/user/js-analyzer",
                    "rules": rules,
                }
            },
            "results": results,
...
def generate_postman_collection(findings: dict, base_url: str = '') -> dict:
    ...
    return {
        "info": {
            "name": "JS Analyzer - Discovered Endpoints",
            "_postman_id": "",
            "schema": "https://schema.getpostman.com/json/collection/v2.1.0/collection.json",
            "description": f"Auto-generated by JS Analyzer v4.0 on {datetime.datetime.now().isoformat()}",
        },
        "item": items,
    }
...
def generate_markdown_summary(findings: dict, target: str, meta: dict = None) -> str:
    ...
    lines = [
        f"# JS Analyzer — Security Summary",
        f"",
        f"**Target:** `{target}`  ",
        f"**Date:** {now}  ",
    ...
    for risk in risk_order:
        ...
        if not items:
            continue
        lines.append(f"## {risk_emoji[risk]} {risk} ({len(items)})")
        lines.append("")
    ...
    lines.append("---")
    lines.append("*Generated by JS Analyzer v4.0*")
    return '\n'.join(lines)
